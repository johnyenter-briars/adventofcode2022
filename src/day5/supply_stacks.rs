...
pub fn supply_stacks_part1(path: &str) -> Result<String, Box<dyn Error>> {
    ...
    while lines.peek().is_some() {
        ...
        let number_crates: i32 = caps.get(1).unwrap().as_str().parse().unwrap();
        let from: i32 = caps.get(2).unwrap().as_str().parse().unwrap();
        let destination: i32 = caps.get(3).unwrap().as_str().parse().unwrap();

        for _ in 0..number_crates {
            let moving = stacks[from as usize - 1].pop().unwrap();
            stacks[destination as usize - 1].push(moving);
        }
    }
